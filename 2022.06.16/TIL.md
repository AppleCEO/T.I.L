...
2022.06.16 과 같은 날짜 타입이나 AF02 와 같은 항공권 번호와 비슷한 형태의 문자열이 보여질 때 화면이 깨지거나 해당 텍스트를 누르면 달력으로 이동되는 등의 이벤트 처리가 자동으로 되었다.
날짜를 보여주기만 하는 페이지인데 날짜를 누르면 달력으로 이동된다던지 이벤트 팝업이 발생하여서 사용하는데 불편하였다.
